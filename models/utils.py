...
def preprocess(image, y, bits, noise=None, cfg=None, dequant_all=None, dequant_class=None):
    if cfg["vae"]["is_wrapping"]:
        image = image / cfg["vae"]["max_abs_train"] 
        image = image + torch.randn_like(image) * dequant_all * cfg["vae"]["noise_scale"]
        return image 
...
def postprocess(image, bits, cfg=None):
    if cfg["vae"]["is_wrapping"]:
        image = image * cfg["vae"]["max_abs_train"]
        return image 
...
def expm(x):
    ...
    x = x / (2 ** scale)
    s = torch.eye(x.size(-1), dtype=x.dtype, device=x.device)
    t = x
    k = 2
